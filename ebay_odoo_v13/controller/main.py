    ...
    def ebay_oauth(self, **kwargs):
        ...
        if resp.status_code == requests.codes.ok:
            ...
            instance_id.write({'dev_id': ebay_oauth[0].dev_id,
                               ...
                               'auth_token_expiry': datetime.now() + timedelta(
                                   seconds=int(post_data.get('expires_in', False))),
                               'refresh_token_expiry': datetime.now() + timedelta(
                                   seconds=int(post_data.get('refresh_token_expires_in', False)))
                               })
            return request.render("ebay_odoo_v13.thankyou_oauth")
        else:
            return request.render("ebay_odoo_v13.error")
