...
class refund_order(models.TransientModel):
        ...
        for invoice in invoice_id:
            if invoice.state == 'paid':
                move_id = move_obj.search(
                    [('ref', 'ilike', invoice.number), ('ref', 'not like', 'reversal of:'),
                     ('ref', 'not like', 'refund of')])
    ...
    def get_activity_options(self):
        ...
        connection_obj = self.env['ebayerp.osv']
        shop_obj = self.shop_id
        activity_options = connection_obj.call(shop_obj.instance_id, 'getActivityOptions', self.name,
    ...
    def provide_seller_info(self):
        ...
        result = connection_obj.call(shop_obj.instance_id, 'provideSellerInfo', self.name, rma,
                                     shop_obj.instance_id.site_id.site)
