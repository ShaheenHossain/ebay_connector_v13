...
class CompleteSale:
    ...
    def Get(self, order_data):
        ...
        api.RequestData = api.RequestData % {'token': self.Session.Token.encode("utf-8"),
                                             ...
                                             'order_id': order_data['ItemID'].encode("utf-8") + '-' + order_data[
                                                 'TransactionID'].encode("utf-8"),
                                             'order_line_item_id': order_data['ItemID'].encode("utf-8") + '-' +
                                                                   order_data['TransactionID'].encode("utf-8"),
                                             }
        ...
        for item in itemlist:
            ...
            container += """
                    ...
                    <PictureDetails>%s</PictureDetails>
                                  ...
                                  "<![CDATA[" + item['description'].encode("utf-8") + "]]>", subtitle,
                                  item['category_code'], Itemspecifics, str(item['best_offer']), item['site_code'],
                                  sku_str, item['qnt'], item['price'], buy_it_now, item['condition'], item['duration'],
                                  item['location'], pickupinstore, item['list_type'], shipping_str, return_policy,
                                  item['country_code'], item['private_listing'], item['hand_time'], item['currency'],
                                  s_time, item['postal_code'], item['payment_method'], item['paypal_email'],
                                  ebay_images)
...
class AddFixedPriceItem:
    ...
    def Get(self, ids, itemlist, siteid):
        ...
        if itemlist[0].get('listing_time'):
            s_time = "<ScheduleTime>%s</ScheduleTime>" % (self.item['listing_time'])
        else:
            s_time = ''
        ...
        api.RequestData = """<?xml version="1.0" encoding="utf-8" ?>
            ...
            <ErrorLanguage>en_US</ErrorLanguage>
            <WarningLevel>High</WarningLevel>%s</AddFixedPriceItemRequest>""" % (
            self.Session.Token.encode("utf-8"), container)
...
class UploadSiteHostedPictures:
    ...
    def Get(self, filename, siteid):
        ...
        string10 = string9 + "<RequesterCredentials><eBayAuthToken>" + self.Session.Token.encode(
            "utf-8") + "</eBayAuthToken></RequesterCredentials>" + '\r\n'
        ...
        for item in itemlist:
            ...
            container += """<AddItemRequestContainer>
                    ...
                    <PictureDetails>%s</PictureDetails>
                                                     ...
                                                     "<![CDATA[" + item['description'].encode("utf-8") + "]]>",
                                                     subtitle, item['category_code'], Itemspecifics,
                                                     str(item['best_offer']), item['site_code'], sku_str, item['qnt'],
                                                     item['price'], buy_it_now, item['condition'], item['duration'],
                                                     item['location'], pickupinstore, item['list_type'], shipping_str,
                                                     return_policy, item['country_code'], item['private_listing'],
                                                     item['hand_time'], item['currency'], s_time, item['postal_code'],
                                                     item['payment_method'], item['paypal_email'], ebay_images)
    ...
    def Get(self, userid, siteid):
        ...
        if responseDOM.getElementsByTagName('Ack')[0].childNodes[0].data == 'Success':
            ...
            raise osv.except_osv('Error !',
                                 responseDOM.getElementsByTagName('Errors')[0].childNodes[0].childNodes[0].data)
            return False
        return responseDOM
