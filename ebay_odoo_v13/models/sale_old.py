...
class sale_shop(models.Model):
    ...
    def get_ebay_store_category(self):
        ...
        if not store_name:
            store_datas = connection_obj.call(inst_lnk, 'GetStore', self.ids, user_id, site_id)
    ...
    def export_stock_and_price(self):
        ...
        if isinstance(listing_ids, list) or isinstance(listing_ids, int):
            listing_browse_data = ebay_prod_list_obj.browse(self._cr, self._uid, listing_ids)
        else:
                ...
                if item_id:
                    ...
                    if len(result):
                        if result[0].get('ItemID', False):
                            ...
                            logger.info("test--------export_stock_and_price---ebay_sku---- %s", ebay_sku)
                            if not ebay_sku:
                                continue

                            if is_variation:
                                variation_data = self.get_variation_data(result[0]['variations'], ebay_sku)
                                logger.info("test--------export_stock_and_price---variation_data---- %s",
                                            variation_data)
