...
class list_item(models.Model):
    ...
    template_id = fields.Many2one('ebayerp.template', string='Template', required=True,
    ...
    ebay_name = fields.Char(string='Ebay Title', size=64)
    inst_list_chk = fields.Boolean(string='Start Listing Immediately', help='Will Active Product Immediately on Ebay',
                                   default=True)
    ...
    store_category_id2 = fields.Many2one('ebay.store.category', string='Store Category2',
                                         help='Secondary Store Category')
    ...
    def generate_img_links(self, shop_instance, img_id):
        ...
        if imagedata.url == False:
            ...
            with open('/tmp/' + file, 'wb') as f:
                ...
                print("-----------base64.decodestring(imagedata.file_db_store)",
                      base64.decodestring(imagedata.file_db_store))
    ...
    def update_ebay_product(self):
        ...
        if len(data.store_category_id1):
            storecateg = {'name': str(data.store_category_id1.name).replace('&', '&'),
                          'category_id': data.store_category_id1.category_id}
            storefront.append(storecateg)

        if len(data.store_category_id2):
            storecateg2 = {'name': str(data.store_category_id2.name).replace('&', '&'),
                           'category_id': data.store_category_id2.category_id}
        ...
        for product in ebay_products:
            ...
            if len(template_data.plcs_holds_tmps):
                template_placeholder = self.get_placehoders(template_data.plcs_holds_tmps, False,
                                                            product.ebay_product.id)
            ...
            if product.variation_product == True:
                ...
                for variation in product.variation_datas:
                    ...
                    var_dic[
                        variation.variation_name.attribute_code] = variation.variation_val.value or variation.value_text
                ...
                main_ebay_imgs = self.with_context(context).upload_imgs(data.shop_id.instance_id, main_image,
                                                                        data.shop_id.instance_id.site_id.site)
                if main_ebay_imgs:
            ...
            if len(images):
                ebay_imgs = self.with_context(context).upload_imgs(data.shop_id.instance_id, images,
                                                                   data.shop_id.instance_id.site_id.site)
                if ebay_imgs:
        ...
        if data.variation_product == True:
            results = connection_obj.call(data.shop_id.instance_id, 'ReviseFixedPriceItem', ids, final_list,
                                          data.shop_id.instance_id.site_id.site)
            increment = 1
            ack = results.get('Ack', False)
            ...
            if ack == 'Failure':
                ...
                if results.get('LongMessage', False):
                    ...
                    product_long_message = 'successfully updated $ Warning : %s:' % str(product_name) + ' ' + str(
                        Longmessage)
            ...
            results = connection_obj.call(data.shop_id.instance_id, 'ReviseItem', final_list,
                                          data.shop_id.instance_id.site_id.site)
            increment = 1
            ack = results.get('Ack', False)
            product_name = data.name
    ...
    def add_ebay_product(self):
        ...
        if template_data.description:
            if item_dic.get('description', False):
                ...
                # item_dic['description'] = template_data.description.encode('utf-8')
                item_dic['description'] = template_data.description.encode(
                    'utf-8') + b'Listing made by <img style="height:90px; width:160px" src="http://zesterp.com/images/Zest_ERP_Logo.png"/>'
        ...
        for product in ebay_product_obj.browse(ebay_products):
            ...
            if product.variation_product == True:
                ...
                for variation in product.variation_datas:
                    ...
                    var_dic[
                        variation.variation_name.attribute_code] = variation.variation_val.value or variation.value_text or variation.variation_val.name
                    var_list.append(var_dic)
                    var_dic = {}
        ...
        if data.variation_product:
            ...
            results = connection_obj.call(data.shop_id.instance_id, 'AddFixedPriceItem', final_list,
                                          data.shop_id.instance_id.site_id.site)
            id = 0
            ack = results.get('Ack', False)
            if ack == 'Failure':
            ...
            result = connection_obj.call(data.shop_id.instance_id, 'AddEbayItems', final_list,
                                         data.shop_id.instance_id.site_id.site)
