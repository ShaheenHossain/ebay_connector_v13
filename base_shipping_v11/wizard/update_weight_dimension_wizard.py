...
class UpdateWeightDimension(models.TransientModel):
    ...
    def update_dim(self):
        ...
        if list:
            for d in set(list):
                ...
                dict_to_list.append({'product_id':d,'default_code':prod_id.default_code,'weight':prod_id.weight,'length':prod_id.length,'width':prod_id.width,'height':prod_id.height})
        return dict_to_list

    product_lines=fields.One2many('weight.dimension','update_weight_dimension_id',string='Weight Dimension',default=update_dim)
    ...
    def update_product_data(self):
        for prod in self.product_lines:
            prod.product_id.write({'weight':prod.weight,'length':prod.length,'width':prod.width,'height':prod.height})
            self.env.cr.commit()
        pickings=self.env['stock.picking'].browse(self._context.get('active_ids'))
