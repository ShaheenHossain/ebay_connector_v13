...
class update_base_picking(models.TransientModel):
    ...
    def create_shipment(self):
        ...
        csv_shipping_dict={}
        csv_shipping_list=[]
        csv_shipment_id=self.env['ir.module.module'].search([('name','=','csv_shipment'),('state','=','installed')])
        if csv_shipment_id:
                    ...
                    if picking.carrier_id.select_service.id in configuration_list:
                        ...
                        if picking.carrier_id.select_service.id in csv_shipping_dict:
                            ...
                            if picking.carrier_id.select_service.id in no_csv_fields:
                                raise UserError(_('CSV Fields not found, please check the CSV Configuration for %s.'%picking.carrier_id.select_service.name))
                            csv_shipping_list=[]
